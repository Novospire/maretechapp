...
4) Parallel-safe by design:
   - Design must not rely on global mutable state (even if CI does not run pytest -n auto yet).
5) CI stays green:
   - Existing tests pass unchanged or with minimal fixture updates.
   - Add at least one explicit regression test (e.g., test_no_global_user_store_leakage) that would fail under the old global-store behavior.
